...
bool Elas::process(const Mat &image_left, const Mat &image_right, Mat &disparity_left, Mat &disparity_right)
{
...

    {
        ...
        cout << "Support Points Amount: " << support_points_.size() << endl;
    }
    ...
    disparity_left.convertTo(disparity_left, CV_8U);
    imshow("Disparity", disparity_left);
    waitKey();
    exit(0);
    return true;
}
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            if (d > 0)
            {
                ...
                for (int d_neighbour_x = i - param_.incon_window_size; d_neighbour_x <= i + param_.incon_window_size; d_neighbour_x++)
                {
                    for (int d_neighbour_y = i - param_.incon_window_size; d_neighbour_y <= i + param_.incon_window_size; d_neighbour_y++)
                    {
                            ...
                            const vector<Vec3d> &triangulate_d, Mat &disparity)
{
    ...
    cout << "Loop Compute" << endl;

    cout << plane_params.size() << " " << triangulate_points.size() << " " << triangulate_d.size() << endl;
    int count = 0;
    for (int i = 0; i < plane_params.size(); i++)
    {
        cout << count << " " << plane_params.size() << endl;
        count++;
        if (!right_image)
        {
                     ...
                     const bool &valid, const bool &right_image, Mat &disparity)
{
    ...
    if (right_image)
    {
        ...
        data_desc2 = descriptor_left_.ptr<uchar>(v * width_ + u);
    }
    else
    {
