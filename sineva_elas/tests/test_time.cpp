...
int main(int argc, char *argv[])
{
  ...
  elas_para["support_threshold"] >> param.support_threshold;
  elas_para["support_texture"] >> param.support_texture;
  elas_para["candidate_stepsize"] >> param.candidate_stepsize;
  elas_para["incon_window_size"] >> param.incon_window_size;
  elas_para["incon_threshold"] >> param.incon_threshold;
  elas_para["incon_min_support"] >> param.incon_min_support;
  elas_para["add_corners"] >> param.add_corners;
  ...
  elas_para["match_texture"] >> param.match_texture;
  elas_para["lr_threshold"] >> param.lr_threshold;
  elas_para["speckle_sim_threshold"] >> param.speckle_sim_threshold;
  elas_para["speckle_size"] >> param.speckle_size;
  elas_para["ipol_gap_width"] >> param.ipol_gap_width;
  elas_para["filter_median"] >> param.filter_median;
  elas_para["filter_adaptive_mean"] >> param.filter_adaptive_mean;
  elas_para["postprocess_only_left"] >> param.postprocess_only_left;
  elas_para["subsampling"] >> param.subsampling;
  ...
  for (int i = 0; i < all_files.size(); ++i)
  {
    ...
    Mat disparity_map = stereo_match.run(left_gray, right_gray, baseline, fx);
