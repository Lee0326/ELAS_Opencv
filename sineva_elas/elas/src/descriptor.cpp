...
{
  Mat descriptor(width_ * height_, 16, CV_16S);

  int step = grad_x.step;
  for (int v = 3; v < height_ - 2; ++v)
  {
    const short *data_x = grad_x.ptr<short>(v);
    const short *data_y = grad_y.ptr<short>(v);

    for (int32_t u = 3; u < width_ - 2; ++u)
    {
      ...
      short *data_desc = descriptor.ptr<short>((v - 3) * (width_ - 4) + u);
